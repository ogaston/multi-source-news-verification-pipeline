...
_CacheKey = tuple[str, tuple[str, ...], int, str, str]
_CACHE: dict[_CacheKey, tuple[float, tuple[SearchResult, ...]]] = {}
_CACHE_LOCK = Lock()
...
def _cache_get(key: _CacheKey, ttl_seconds: int) -> list[SearchResult] | None:
    now = time.monotonic()
    with _CACHE_LOCK:
...
def _cache_put(key: _CacheKey, results: list[SearchResult]) -> None:
    with _CACHE_LOCK:
