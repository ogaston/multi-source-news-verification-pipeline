...
from audit.agents.fact_checker import run, validate_fact_check
from audit.search import SearchProviderError, SearchResult
from audit.state import ExtractedClaim
...
def test_run_searches_verifiable_claims_only_and_injects_evidence():
    ...
    with (
        patch("audit.agents.fact_checker.search_domains", return_value=[_result()]) as search,
        patch("audit.agents.fact_checker.invoke_llm", return_value=llm_payload) as invoke,
    ):
...
def test_run_search_failure_stops_more_calls_and_forces_insufficient_evidence():
    ...
    with (
        patch(
            "audit.agents.fact_checker.search_domains",
            side_effect=SearchProviderError("unavailable"),
        ) as search,
        patch("audit.agents.fact_checker.invoke_llm", return_value=attempted),
    ):
