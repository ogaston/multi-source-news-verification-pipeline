...
def test_landing_page_uses_env_key(monkeypatch):
    monkeypatch.setenv("MCP_API_KEY", "runtime-key")

    html = render_landing_html()

    assert "Authorization: Bearer runtime-key" in html
    assert "{{MCP_API_KEY}}" not in html
