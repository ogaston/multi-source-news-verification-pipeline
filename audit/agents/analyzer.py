...
from audit.state import AnalysisOutput, ExtractedClaims, StoryAuditState
from audit.utils import parse_llm_json
...
def parse_analysis_output(text: str | None) -> AnalysisOutput | None:
    ...
    if not raw:
        return None
    try:
        return parse_llm_json(raw, AnalysisOutput)
...
def run(state: StoryAuditState) -> dict:
    ...
    return {"analysis": parse_analysis_output(content)}
