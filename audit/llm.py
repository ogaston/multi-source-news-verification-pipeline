...
from common.config import get_deepseek_config

load_dotenv()
# Optional local override for agent-only runs.
load_dotenv(Path(__file__).resolve().parent / ".env", override=True)

_, DEEPSEEK_MODEL, DEEPSEEK_BASE_URL, DEEPSEEK_MAX_TOKENS, MAX_RETRIES, DEFAULT_RETRY_WAIT = get_deepseek_config()

_RETRY_WAIT_RE = re.compile(r"try again in ([\d.]+)\s*s", re.IGNORECASE)


def get_llm(*, temperature: float = 0) -> ChatOpenAI:
    api_key = get_deepseek_config()[0]
    if not api_key:
        raise SystemExit(
            "DEEPSEEK_API_KEY is required (set it in .env or the environment)"
        )
    return ChatOpenAI(
