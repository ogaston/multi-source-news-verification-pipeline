...
def _extract_direct_paragraphs(container) -> str:
    paragraphs = container.find_all("p", class_="paragraph", recursive=False)
    if not paragraphs:
...
class ElNacionalProvider(BaseNewsProvider):
    ...
    def get_date(self, result: CrawlResult) -> str:
        for html in (result.html, result.cleaned_html):
            ...
            date_time = soup.select_one("a.c-detail__info__more__date time[datetime]")
            if date_time and date_time.get("datetime"):
                return date_time["datetime"].strip()

            date_el = soup.select_one("a.c-detail__info__more__date")
    ...
    def get_content(self, result: CrawlResult) -> str:
        ...
        if html:
            ...
            if content_el:
                ...
                content = _extract_direct_paragraphs(content_el)
                if content:
                    return content
    ...
    def is_valid_url(self, url: str) -> bool:
        ...
        if parsed.scheme not in {"http", "https"}:
            return False
        if parsed.netloc != "elnacional.com.do":
            return False
